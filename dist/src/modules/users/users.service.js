...
let UsersService = class UsersService {
    ...
    async update(id, dto) {
        ...
        if (dto.email) {
            ...
            if (existing && existing.id !== id) {
                throw new common_1.ConflictException(`Email "${dto.email}" is already taken`);
            }
        }
    ...
    async getPublicProfile() {
        ...
        if (!profile) {
            throw new common_1.NotFoundException('Public portfolio profile has not been initialized');
        }
        return profile;
    }
};
